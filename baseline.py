...
#user_app_usage = pd.read_csv(data_path+'user_app_usage1.csv')
...
def processUserAppUsage():
    ...
    resTable.to_csv(feature_path+'appuseProcessed.csv',index=0)
...
#读取数据
app_info = pd.read_csv(data_path+'app_info.csv', names=['appId','kinds'])
...
size = len(user_app_actived_list)
h = len(app_info_list)
...
for i in range(0, size):
	matrix[i][0] = user_app_actived_list[i][0]

for i in range(0, size):
	a = user_app_actived_list[i][1].split('#')#第一行的appid的分割
	length = len(a)#appid长度
	for j in range(0, length):#检查appid相对应的类型
		for k in range(0, h):#app_info表的匹配
			if a[j] == b[k][0]:
				if b[k][1] == "商务":
					...
					matrix[i][11] = matrix[i][11] + 1
				elif b[k][1] == "体育竞速":
					matrix[i][12] = matrix[i][12] + 1
				elif b[k][1] == "社交通讯":
					matrix[i][13] = matrix[i][13] + 1
				elif b[k][1] == "购物比价":
					matrix[i][14] = matrix[i][14] + 1
				elif b[k][1] == "角色扮演":
					matrix[i][15] = matrix[i][15] + 1
				elif b[k][1] == "出行导航":
					matrix[i][16] = matrix[i][16] + 1
				elif b[k][1] == "棋牌桌游":
					matrix[i][17] = matrix[i][17] + 1
				elif b[k][1] == "拍摄美化":
					matrix[i][18] = matrix[i][18] + 1
				elif b[k][1] == "儿童":
					matrix[i][19] = matrix[i][19] + 1
				elif b[k][1] == "旅游住宿":
					matrix[i][20] = matrix[i][20] + 1
				elif b[k][1] == "汽车":
					matrix[i][21] = matrix[i][21] + 1
				elif b[k][1] == "美食":
					matrix[i][22] = matrix[i][22] + 1
				elif b[k][1] == "主题个性":
					matrix[i][23] = matrix[i][23] + 1
				elif b[k][1] == "网络游戏":
					matrix[i][24] = matrix[i][24] + 1
				elif b[k][1] == "学习办公":
					matrix[i][25] = matrix[i][25] + 1
				elif b[k][1] == "模拟游戏":
					matrix[i][26] = matrix[i][26] + 1
				elif b[k][1] == "休闲游戏":
					matrix[i][27] = matrix[i][27] + 1
				elif b[k][1] == "棋牌天地":
					matrix[i][28] = matrix[i][28] + 1
				elif b[k][1] == "体育射击":
					matrix[i][29] = matrix[i][29] + 1
				elif b[k][1] == "策略游戏":
					matrix[i][30] = matrix[i][30] + 1
				elif b[k][1] == "角色游戏":
					matrix[i][31] = matrix[i][31] + 1
				elif b[k][1] == "医疗健康":
					matrix[i][32] = matrix[i][32] + 1
				elif b[k][1] == "休闲娱乐":
					matrix[i][33] = matrix[i][33] + 1
				elif b[k][1] == "动作冒险":
					matrix[i][34] = matrix[i][34] + 1
				elif b[k][1] == "主题铃声":
					matrix[i][35] = matrix[i][35] + 1
				elif b[k][1] == "图书阅读":
					matrix[i][36] = matrix[i][36] + 1
				elif b[k][1] == "电子书籍":
					matrix[i][37] = matrix[i][37] + 1
				elif b[k][1] == "益智棋牌":
					matrix[i][38] = matrix[i][38] + 1
				elif b[k][1] == "合作壁纸*":
					matrix[i][39] = matrix[i][39] + 1
				elif b[k][1] == "表盘个性":
					matrix[i][40] = matrix[i][40] + 1

def mergeAppData(baseTable):
    resTable = baseTable.merge(matrix, how='left', on='uid')
    appusedTable = pd.read_csv(feature_path+'appuseProcessed.csv')
